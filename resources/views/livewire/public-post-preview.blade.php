<div class="fixed inset-0 z-50 bg-black flex flex-col h-[100dvh]"
     x-data="{ 
        ...
        init() {
            this.currentIndex = {{ $initialMediaIndex }};
            {{-- Watch for Livewire changes --}}
            this.$watch('currentIndex', value => {
                if(value >= this.allMedia.length) this.currentIndex = Math.max(0, this.allMedia.length - 1);
            });
        },
        ...
        archive() {
            let currentMedia = this.allMedia[this.currentIndex];
            if (currentMedia) {
                $wire.archiveMedia(currentMedia.id).then(() => {
                    this.showConfirm = false;
                    {{-- Local sync after delete --}}
                    this.allMedia = this.allMedia.filter(m => m.id !== currentMedia.id);
                    if (this.currentIndex >= this.allMedia.length) {
    ...
    <header class="p-6 flex items-center justify-between z-50">
        ...
        <div class="flex gap-1.5 px-3 py-1.5 bg-white/5 backdrop-blur-md rounded-full border border-white/10">
            <template x-for="(m, index) in allMedia" :key="m.id">
        ...
        <div class="relative w-full h-full flex items-center justify-center p-4">
            <template x-for="(m, index) in allMedia" :key="m.id">
                <div x-show="currentIndex === index" 
                     class="w-full h-full flex items-center justify-center animate-in fade-in zoom-in-95 duration-500">
    ...
    <div class="p-8 pb-12 flex flex-col items-center gap-8">
        ...
        <div class="text-center space-y-1">
            <template x-if="allMedia[currentIndex]">
                <div class="animate-in fade-in slide-in-from-bottom-2 duration-500">
                    <p x-show="allMedia[currentIndex].location" x-text="allMedia[currentIndex].location" class="text-sm font-bold text-white lowercase tracking-tight"></p>
                    <p x-text="allMedia[currentIndex].date" class="text-[10px] text-white/40 uppercase tracking-widest font-bold"></p>
                </div>
            </template>
        </div>
        ...
        <div class="relative w-10 h-10">
            <template x-if="allMedia[currentIndex] && allMedia[currentIndex].user_id === {{ Auth::id() }}">
                <button @click="showConfirm = true" class="absolute inset-0 rounded-full bg-red-500/10 backdrop-blur-xl border border-red-500/20 flex items-center justify-center text-red-500 transition-transform active:scale-90 z-40">
