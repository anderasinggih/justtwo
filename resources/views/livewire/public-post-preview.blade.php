        ...
        <div class="relative w-10 h-10">
            @foreach($allMedia as $index => $m)
                @if(Auth::check() && $m['user_id'] === Auth::id())
                    <button x-show="currentIndex === {{ $index }}" wire:click="archiveMedia({{ $m['id'] }})" wire:confirm="move this photo to archive? (it will be deleted forever in 30 days)" class="absolute inset-0 rounded-full bg-red-500/10 backdrop-blur-xl border border-red-500/20 flex items-center justify-center text-red-500 transition-transform active:scale-90 z-40">
